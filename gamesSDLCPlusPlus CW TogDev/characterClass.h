...
public:
	characterClass() {
		...
		this->characterImg[_direction::down] = this->loadOneImg("./images/character/ .png");
		this->characterImg[_direction::up] = this->loadOneImg("./images/character/ .png");
		this->characterImg[_direction::left] = this->loadOneImg("./images/character/ .png");
		this->characterImg[_direction::right] = this->loadOneImg("./images/character/ .png");
		this->characterImgSize = gameSettings::winSetting.cellSize;
		this->chRect = { this->charactI,this->charactI, this->characterImgSize,this->characterImgSize };
	}
	...
	void transmit(int direction, std::vector<std::vector<cell>>&V) {
		...
		case _direction::up:
			if (this->charactI- 1 > 0 )
			{
				...
				{
					this->charactI--;
					this->chRect.y = this->charactI * this->characterImgSize;
		...
		case _direction::left:
			if (this->charactJ - 1 > 0)
			{
				...
				{
					this->charactJ--;
					this->chRect.y = this->charactJ * this->characterImgSize;
					this->_blit(characterImg[_direction::up], this->chRect);
					SDL_UpdateWindowSurface(gameSettings::winSetting.win);
				}
			}
